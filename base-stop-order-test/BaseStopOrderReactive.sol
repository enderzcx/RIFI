...
contract BaseStopOrderReactive is IReactive, AbstractReactive {
    event CallbackSent();
    event Done();
    ...
    function react(LogRecord calldata log) external vmOnly {
        ...
        if (log._contract == stop_order) {
            ...
            BaseReserves memory sync = abi.decode(log.data, ( BaseReserves ));
    ...
    function below_threshold(BaseReserves memory sync) internal view returns (bool) {
        if (token0) {
